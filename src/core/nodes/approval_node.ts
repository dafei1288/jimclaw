...
function isCheckpointApproved(
  customerApprovalState: CustomerApprovalState | null | undefined,
  stage: ApprovalStage
): boolean {
  return Boolean(customerApprovalState?.checkpoints?.find((item) => item.stage === stage)?.approved);
}
...
export async function approvalNode(
  ...
  if (isCheckpointApproved(customerApprovalState, stage)) {
    const result = {
      requiresApproval: false,
      pendingApprovalStage: null,
      approvalNextNode: nextNode,
